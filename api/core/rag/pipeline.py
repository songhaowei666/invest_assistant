...
class Pipeline:
    ...
    def download_docling_models(): 
        ...
        parser = PDFParser(output_dir=rag_dir)
        parser.parse_and_export(input_doc_paths=[rag_dir / "dummy_report.pdf"])
...
# 调试启动：VS Code/Cursor 选择 launch「RAG: Pipeline (core.rag.pipeline)」后 F5
# 命令行：cd api && python -m core.rag.pipeline
# 只需取消你想运行的方法的注释即可
# 你也可以修改 run_config 以尝试不同的配置
if __name__ == "__main__":
    # 设置数据集根目录（此处以 test_set 为例）
    root_path = Path(__file__).resolve().parent.parent.parent / "data" / "stock_data"
