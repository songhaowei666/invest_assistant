"""
在 PostgreSQL 中创建或确认存在 scheduled_task、scheduled_task_run 表，并写入表/列 COMMENT。

可重复执行：表已存在时不报错；每次执行会按 ORM 元数据刷新 COMMENT。
用法: cd api && python3 scripts/create_scheduled_tasks_tables.py
"""
...
from models.scheduled_task import ScheduledTask, ScheduledTaskRun  # noqa: E402

TABLES = (ScheduledTask, ScheduledTaskRun)
