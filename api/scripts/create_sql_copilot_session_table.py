"""
在 PostgreSQL 中创建或确认存在 sql_copilot_session 表，并写入表/列 COMMENT。

可重复执行：表已存在时不报错；每次执行会按 ORM 元数据刷新 COMMENT。
依赖 api/.env 中的 DATABASE_URL 或 DB_*（与 configs.config.Settings 一致）。
用法: cd api && python3 scripts/create_sql_copilot_session_table.py
"""
...
from models.sql_copilot_session import SqlCopilotSession  # noqa: E402
...
def main() -> None:
    ...
    table = SqlCopilotSession.__table__
